...
async function createService(name: string) {
    ...
    //check Services Dir exists
    const ServicePath = path.join(rootPath, `${configData?.paths.services}`);
    if (!fs.existsSync(ServicePath)) {
        fs.mkdirSync(ServicePath, { recursive: true });
    }
    const filePath = path.join(rootPath, `${configData?.paths.services}`, `${name}.${configData?.lang}`);
    if (fs.existsSync(filePath)) {
        console.log(`Service ${name}.${configData?.lang} already exists!`.red);
        return;
    } else {
        const slice = name.split('/');
        console.log(`Creating Service ${slice[slice.length - 1]}...`.yellow);
        ...
        slice.pop();
        if (fs.existsSync(path.join(rootPath, `${configData?.paths.services}`, slice.join('/')))) {
        ...
        if (fs.existsSync(filePath)) {
            console.log(`Service ${name}.${configData?.lang} created successfully!`.green);
        } else {
            console.log(`Failed to create Service ${name}!`.red);
        }
    }
}
