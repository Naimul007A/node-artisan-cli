function ControllerContent(name: string, type: string = "ts", service: boolean = false, serviceName: string = "") {
    ...
    const tsControllerContent = `import { Request, Response, NextFunction } from "express";
...
async function ${name}Destroy(req, res, next) {
    ...
    `;
    const serviceTsControllerContent = `import { Request, Response, NextFunction } from "express";
import createError from "http-errors";
import { ${Service.toLowerCase()}Index } from "@services/${serviceName}";
/**
* Display a listing of the resource.
*/
async function ${name}Index(req: Request, res: Response, next: NextFunction) {
    try {
       const ${name}s = await ${Service.toLowerCase()}Index();
        res.status(200).json({
            success: {
                message: "${name}s fetched successfully.",
                data: ${name}s,
...
export { ${name}Index, ${name}Store, ${name}Show, ${name}Update, ${name}Destroy };
    `;
    const serviceJsControllerContent = `
import createError from "http-errors";
import { ${Service.toLowerCase()}Index } from "@/services/${serviceName}";
/**
* Display a listing of the resource.
...
async function ${name}Destroy(req, res, next) {
    ...
    if (type === "ts") {
        ...
        if (service) {
            return serviceJsControllerContent;
        } else {
